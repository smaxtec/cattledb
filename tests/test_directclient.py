...
class DirectclientTests(unittest.TestCase):
    ...
    def test_events(self):
        ...
        res = client.delete_events(
            "device1",
            "upload",
            pendulum.datetime(2015, 2, 5, 12, 0, tz="UTC"),
        ...
        self.assertEqual(
            res[0].ts, pendulum.datetime(2015, 2, 5, 12, 0, tz="UTC").int_timestamp
        )
        self.assertEqual(res[0].value["foo1"], "bar1")
        self.assertEqual(
            res[1].ts, pendulum.datetime(2015, 2, 5, 13, 0, tz="UTC").int_timestamp
        )
        ...
        self.assertEqual(
            res[0].ts, pendulum.datetime(2015, 2, 6, 12, 0, tz="UTC").int_timestamp
        )
    ...
    def test_activity(self):
        ...
        client.incr_activity(
            "reader1",
            "dev1",
            pendulum.datetime(2018, 2, 5, 12, 0, tz="UTC").int_timestamp,
            parent_ids=["parent1", "parent2"],
        )
        client.incr_activity(
            "reader1",
            "dev1",
            pendulum.datetime(2018, 2, 5, 13, 0, tz="UTC").int_timestamp,
            parent_ids=["parent1"],
        )
        client.incr_activity(
            "reader1",
            "dev1",
            pendulum.datetime(2018, 2, 5, 15, 0, tz="UTC").int_timestamp,
            parent_ids=["parent1"],
        )
        client.incr_activity(
            "reader2",
        ...
        client.incr_activity(
            "reader2",
            "dev1",
            pendulum.datetime(2018, 2, 4, 12, 0, tz="UTC").int_timestamp,
            parent_ids=["parent1", "parent2"],
            value=10,
        )

        res = client.get_total_activity(
            pendulum.datetime(2018, 2, 5, 12, 0, tz="UTC").int_timestamp
        )
        ...
        res = client.get_day_activity(
            "parent2", pendulum.datetime(2018, 2, 5, 12, 0, tz="UTC").int_timestamp
        )
        self.assertEqual(len(res), 2)
        self.assertEqual(res[0].day_hour, "2018020512")
        self.assertEqual(res[0].reader_id, "reader1")
        ...
        res = client.get_reader_activity(
            "reader2",
            pendulum.datetime(2018, 2, 4, 12, 0, tz="UTC").int_timestamp,
            pendulum.datetime(2018, 2, 5, 15, 0, tz="UTC").int_timestamp,
        )
