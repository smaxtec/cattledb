...
class TimeSeriesStorageTest(unittest.TestCase):
    ...
    def test_simple(self):
        ...
        r = db.timeseries.delete_timeseries(
        ...
        r = db.timeseries.get_single_timeseries(
            "sensor1", "temp", t + 24 * 60 * 60, t + 24 * 60 * 60 + 500 * 600
        )
        a = list(r.all())
        d = list(r.aggregation("daily", "mean"))
    ...
    def test_delete(self):
        ...
        data_list = [
            (base - datetime.timedelta(minutes=10 * x), random.random() * 5)
        ...
        r = db.timeseries.delete_timeseries(
            "device", ["ph"], from_ts - 24 * 60 * 60, to_ts + 24 * 60 * 60
        )
    ...
    def test_large(self):
        ...
        for id in ["sensor41", "sensor45", "sensor23", "sensor47"]:
            ...
            data = [
                {"key": id, "metric": "act", "data": d1},
                {"key": id, "metric": "temp", "data": d2},
                {"key": id, "metric": "ph", "data": d3},
            ]
            db.timeseries.insert_bulk(data)

        r = db.timeseries.get_timeseries(
            "sensor47", ["act", "temp", "ph"], start, start + 600 * 4999
        )
    ...
    def test_selective_delete(self):
        ...
        ph_data = [
            (base - datetime.timedelta(minutes=10 * x), ((x % 3) + 4))
            for x in range(0, 144 * 5)
        ]
        act_data = [
            (base - datetime.timedelta(minutes=10 * x), ((x % 3) + 20))
            for x in range(0, 144 * 5)
        ]
        ...
        # delete all data just in case
        r = db.timeseries.delete_timeseries(
            "dev1", ["act", "ph"], from_ts - 24 * 60 * 60, to_ts + 24 * 60 * 60
        )
        ...
        self.assertEqual(
            len(get_timeseries("dev1", "ph", from_ts, from_ts + 24 * 60 * 60 - 1)), 0
        )
        ...
        r = db.timeseries.delete_timeseries(
            "dev1", ["act"], delete_start + 12 * 60 * 60, delete_end - 12 * 60 * 60
        )
        ...
        self.assertEqual(
            len(get_timeseries("dev1", "act", delete_start, delete_end - 1)), 0
        )
